...
- **A documented setup path produced a stack with empty secrets, and started it anyway.**
  `CONTRIBUTING.md` offered `cp .env.example .env` as an alternative to the installer. The example
  ships every secret empty, compose interpolated them without complaint, and the result was a
  running stack whose session signing key, mail-server admin secret, OAuth client secret and
  Keycloak/database passwords were all `""`. Nothing downstream caught it: the pinned webmail image
  starts with `SESSION_SECRET=""`, answers HTTP 307 and logs nothing — measured, not assumed.
  Both compose files now mark those five variables required (`${VAR:?…}`); `docker compose` refuses
  and names the variable (measured **exit 15**). What is *not* claimed is that the empty key was
  exploitable — nothing in the third-party image references it where that could be established.
  The defect is shipping a documented path to a silently misconfigured stack.
- **Only JMAP may reach the mail server through nginx, and now something checks it.** Both nginx
  templates state that the admin API — which returns account passwords in cleartext — must never be
  reachable from the internet. The suite asserted eight *other* nginx properties and not that one,
  so widening the location to `^/(jmap|api)` would have passed all 196 checks. It now fails, which
  was verified by making the change and watching it go red before reverting it.
- **CI checkouts no longer persist the job token.** `scorecard.yml` and `pin-currency.yml` already
  refused it; `ci.yml`'s five checkouts were the odd ones out, leaving a credential in `.git/config`
...
### Added
- **The project has a logo, and the READMEs lead with it.** All twelve of them, English and the
  eleven translations, through `<picture>` so dark-mode readers get a variant whose navy wordmark
  has been lifted to a readable blue — the original disappears against a dark page. The supplied
  artwork was a presentation mockup (logo printed on textured paper, drop shadow, vignette, ghosted
  corner watermarks); the mark and wordmark were isolated onto transparency and everything the
  mockup added was removed. All three files are stripped of metadata: the source carried C2PA
  content credentials, which are provenance for the artwork and not for this repository.
  `NOTICE` now reserves the name and the marks — the MIT grant covers the code, not the identity,
  and a fork is welcome under its own name.
- **Releases carry artefacts, and the artefacts carry provenance.** `release.yml` attaches a
  reproducible `git archive` tarball, an SPDX SBOM of the five pinned images, `SHA256SUMS`, and a
  SLSA provenance bundle (`*.intoto.jsonl`) signed through GitHub's OIDC identity. `SECURITY.md`
  previously argued for shipping nothing; that section now records why the position changed, and
  which part of the old argument was wrong. Git tags remain unsigned, deliberately and on the
  record. The SBOM is generated by `scripts/make_sbom.py`, which states nothing about the
  components itself — images come from the compose files, versions and digests from `.env.example`,
  licences from `scripts/pin_sources.json`.
- **The published history is audited on a schedule.** `check_disclosure.py --history` existed and
  had no trigger, no schedule and no owner. `history-audit.yml` now runs it weekly against
  `scripts/disclosure_history_baseline.json`, which records the 24 findings that are already public
  and cannot be unpublished — rewriting history would not remove them, since the old blobs stay
  reachable by SHA until the host garbage-collects them. New findings fail; a baseline entry that
  stops matching is reported as stale rather than carried.
- **Binaries are scanned for what they publish.** `check_disclosure.py` skipped them with a comment
  claiming they were "covered by the metadata audit". No such audit existed anywhere in the
  repository, so the files most able to carry metadata nobody reads were the only ones nothing
  looked at. Their printable runs are now scanned like any other file; the four tracked binaries
  come back clean.
